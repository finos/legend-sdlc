...
public class ServicesGenerationMojo extends AbstractMojo
{
    ...
    private void generateServices(MutableMap<String, Service> servicesByPath, PureModel pureModel, int parallelism)
    {
        ...
        Object enumWriteLock = new Object();
        Object serviceProviderWriteLock = new Object();

        int effectiveParallelism = Math.min(parallelism, servicesByPath.size());
        if (effectiveParallelism <= 1)
        {
            servicesByPath.forEachValue(s -> generateService(s, pureModel, jsonMapper, extensions, enumWriteLock, serviceProviderWriteLock));
            return;
        }

        getLog().info("Generating services in parallel with parallelism level " + effectiveParallelism);
        ForkJoinPool pool = createForkJoinPool(effectiveParallelism);
        try
        {
            pool.submit(() -> StreamSupport.stream(Arrays.spliterator(servicesByPath.values().toArray(new Service[servicesByPath.size()])), true)
                    .forEach(s -> generateService(s, pureModel, jsonMapper, extensions, enumWriteLock, serviceProviderWriteLock)))
                    .join();
        }
        finally
        {
    ...
    private void generateService(Service service, PureModel pureModel, JsonMapper jsonMapper, MutableList<PlanGeneratorExtension> extensions, Object enumWriteLock, Object serviceProviderWriteLock)
    {
        String path = service.getPath();
        getLog().info("Generating execution artifacts for " + path);
        ...
        try
        {
            ServiceExecutionGenerator generator = ServiceExecutionGenerator.newBuilder()
                    ...
                    .withPlanGeneratorExtensions(extensions)
                    .withEnumWriteLock(enumWriteLock)
                    .withServiceProviderWriteLock(serviceProviderWriteLock)
                    .build();
            generator.generate();
        }
        catch (Exception e)
        {
            String message = "Error generating execution artifacts for " + path;
            getLog().error(message, e);
            throw new RuntimeException(message, e);
        }
        long serviceEnd = System.nanoTime();
        getLog().info(String.format("Finished generating execution artifacts for %s (%.9fs)", path, (serviceEnd - serviceStart) / 1_000_000_000.0));
    }
