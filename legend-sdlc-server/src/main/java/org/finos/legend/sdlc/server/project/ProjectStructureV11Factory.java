...
public class ProjectStructureV11Factory extends ProjectStructureVersionFactory
{
    ...
    @Override
    protected ProjectStructure createProjectStructure(ProjectConfiguration projectConfiguration, ProjectStructureExtensions projectStructureExtensions)
    {
        return new ProjectStructureV11(projectConfiguration, projectStructureExtensions);
    }
    ...
    public static class ProjectStructureV11 extends MultiModuleMavenProjectStructure
    {
        ...
        private ProjectStructureV11(ProjectConfiguration projectConfiguration, ProjectStructureExtensions projectStructureExtensions)
        {
            super(projectConfiguration, ENTITIES_MODULE_NAME, getEntitySourceDirectories(projectConfiguration), OTHER_MODULES.castToMap(), false, projectStructureExtensions);
        ...
        {
            if (this.getProjectStructureExtensions() != null && this.getProjectStructureExtensions().getCollectionExtensions().containsKey(extensionName))
            {
                ProjectStructureExtensions.CollectionMetadata collectionMetadata = this.getProjectStructureExtensions().getCollectionExtensions().get(extensionName);
                ProjectStructureExtensions.PlatformMetadata platformMetadata = this.getProjectStructureExtensions().getPlatformMetadata(collectionMetadata.getPlatform());
                String groupId = platformMetadata.getGroupId();
                String artifactId = collectionMetadata.getArtifactId();
                String versionId = includeVersion ? this.getPlatformPropertyReference(platformMetadata.getName()) : null;
