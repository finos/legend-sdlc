...
@Path("/projects/{projectId}/patches/{patchReleaseVersionId}/groupWorkspaces/{workspaceId}/conflictResolution/revisions/{revisionId}/entities")
@Api("Conflict Resolution")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class ConflictResolutionPatchesGroupWorkspaceRevisionEntitiesResource extends EntityAccessResource
{
    private final EntityApi entityApi;

    @Inject
    public ConflictResolutionPatchesGroupWorkspaceRevisionEntitiesResource(EntityApi entityApi)
    {
        this.entityApi = entityApi;
    }

    @GET
    @ApiOperation("Get entities of the group workspace with conflict resolution at the revision for patch release version")
                                       ...
                                       @ApiParam("If true, exclude invalid entities and return valid entities only. If false, the endpoint will return an error if there are any invalid entities.") boolean excludeInvalid)
    {
        ...
        return executeWithLogging(
                "getting entities in revision " + revisionId + " of group workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersionId,
                () -> getEntities(this.entityApi.getWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, SourceSpecification.newGroupWorkspaceSourceSpecification(projectId, workspaceId, versionId), revisionId), classifierPaths, packages, includeSubPackages, nameRegex, stereotypes, taggedValueRegexes, excludeInvalid)
        );
    }

    @GET
    @Path("{path}")
    @ApiOperation("Get an entity of the group workspace with conflict resolution at the revision by its path for patch release version")
                                  ...
                                  @PathParam("path") String path)
    {
        ...
        return executeWithLogging(
                "getting entity " + path + " in revision " + revisionId + " of group workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersionId,
                () -> this.entityApi.getWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, SourceSpecification.newGroupWorkspaceSourceSpecification(projectId, workspaceId, versionId), revisionId).getEntity(path)
        );
    }
}
