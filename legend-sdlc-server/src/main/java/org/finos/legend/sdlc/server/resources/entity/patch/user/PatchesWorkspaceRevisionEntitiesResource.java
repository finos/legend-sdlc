...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/revisions/{revisionId}/entities")
@Api("Entities")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class PatchesWorkspaceRevisionEntitiesResource extends EntityAccessResource
{
    private final EntityApi entityApi;

    @Inject
    public PatchesWorkspaceRevisionEntitiesResource(EntityApi entityApi)
    {
        this.entityApi = entityApi;
    }

    @GET
    @ApiOperation("Get entities of the workspace at the revision for patch release version")
                                       ...
                                       @ApiParam("If true, exclude invalid entities and return valid entities only. If false, the endpoint will return an error if there are any invalid entities.") boolean excludeInvalid)
    {
        ...
        return executeWithLogging(
                "getting entities in revision " + revisionId + " of workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> getEntities(this.entityApi.getWorkspaceRevisionEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER, revisionId), classifierPaths, packages, includeSubPackages, nameRegex, stereotypes, taggedValueRegexes, excludeInvalid)
        );
    }

    @GET
    @Path("{path}")
    @ApiOperation("Get an entity of the workspace at the revision by its path for patch release version")
                                  ...
                                  @PathParam("path") String path)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "getting entity " + path + " in revision " + revisionId + " of workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.entityApi.getWorkspaceRevisionEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER, revisionId).getEntity(path)
        );
    }
}
