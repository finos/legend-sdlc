...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/groupWorkspaces/{workspaceId}/pureModelContextData")
...
public class PatchesGroupWorkspacePureModelContextDataResource extends PureModelContextDataResource
{
    ...
    public PureModelContextData getPureModelContextData(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId)
    {
        ...
        return executeWithLogging(
                "getting Pure model context data for group workspace " + workspaceId + " in project " + projectId + " for patch release version " + patchReleaseVersion,
                () ->
                {
                    Revision revision = this.revisionApi.getWorkspaceRevisionContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP).getCurrentRevision();
                    if (revision == null)
                    {
                    ...
                    return getPureModelContextData(projectId, revision.getId(), this.entityApi.getWorkspaceEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP));
                });
    }
}
