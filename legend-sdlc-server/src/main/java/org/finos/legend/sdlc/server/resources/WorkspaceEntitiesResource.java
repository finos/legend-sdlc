...
public class WorkspaceEntitiesResource extends EntityAccessResource
{
    ...
    @GET
    @ApiOperation("Get entities of the workspace")
    ...
    public Revision deleteEntities(@PathParam("projectId") String projectId, @PathParam("workspaceId") String workspaceId, DeleteEntitiesCommand command)
    {
        ...
        return (entityPathsToDelete == null) ?
                executeWithLogging(
                        "deleting all entities in workspace " + workspaceId + " for project " + projectId,
                        () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, workspaceId).deleteAllEntities(command.getMessage())
                ) :
                executeWithLogging(
                        "deleting " + entityPathsToDelete.size() + " entities in workspace " + workspaceId + " for project " + projectId,
                        () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, workspaceId).deleteEntities(entityPathsToDelete, command.getMessage())
                );
    }
    ...
    {
        return executeWithLogging(
                "getting entity " + path + " in workspace " + workspaceId + " for project " + projectId,
                () -> this.entityApi.getWorkspaceEntityAccessContext(projectId, workspaceId).getEntity(path)
        );
    }
