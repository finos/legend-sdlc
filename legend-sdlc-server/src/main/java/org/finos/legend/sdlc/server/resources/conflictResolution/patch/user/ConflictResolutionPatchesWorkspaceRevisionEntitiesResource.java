...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/conflictResolution/revisions/{revisionId}/entities")
...
public class ConflictResolutionPatchesWorkspaceRevisionEntitiesResource extends EntityAccessResource
{
                                       ...
                                       @ApiParam("If true, exclude invalid entities and return valid entities only. If false, the endpoint will return an error if there are any invalid entities.") boolean excludeInvalid)
    {
        ...
        return executeWithLogging(
                "getting entities in revision " + revisionId + " of user workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> getEntities(this.entityApi.getWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER, revisionId), classifierPaths, packages, includeSubPackages, nameRegex, stereotypes, taggedValueRegexes, excludeInvalid)
        );
    }
                                  ...
                                  @PathParam("path") String path)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "getting entity " + path + " in revision " + revisionId + " of user workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.entityApi.getWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER, revisionId).getEntity(path)
        );
    }
}
