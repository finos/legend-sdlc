...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/entities/{path}/revisions")
@Api("Revisions")
...
public class PatchEntityRevisionsResource extends BaseResource
{
                                       ...
                                       @QueryParam("limit") @ApiParam("If not provided or the provided value is non-positive, no filtering will be applied") Integer limit)
    {
        ...
        return executeWithLogging(
                "getting revisions for entity " + path + " in project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getProjectEntityRevisionContext(projectId, path, patchReleaseVersion).getRevisions(null, ResolvedInstant.getResolvedInstantIfNonNull(since), ResolvedInstant.getResolvedInstantIfNonNull(until), limit)
        );
    }
                                ...
                                @PathParam("revisionId") @ApiParam("Including aliases: head, latest, current, base") String revisionId)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "getting revision " + revisionId + " for entity " + path + " in project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getProjectEntityRevisionContext(projectId, path, patchReleaseVersion).getRevision(revisionId)
        );
    }
}
