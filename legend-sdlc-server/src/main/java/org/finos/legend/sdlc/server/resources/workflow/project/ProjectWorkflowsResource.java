...
public class ProjectWorkflowsResource extends BaseResource
{
    private final WorkflowApi workflowApi;

    @Inject
    public ProjectWorkflowsResource(WorkflowApi workflowApi)
    {
        this.workflowApi = workflowApi;
    }

    @GET
    @ApiOperation(value = "Get workflows for a project", notes = "Get workflows for a project. If status is provided, then only workflows with the given status are returned. Otherwise, all workflows are returned. If status is UNKNOWN, results are undefined.")
                                       ...
                                       @QueryParam("limit") @ApiParam("Limit the number of workflows returned (if not provided or the provided value is non-positive, no filtering will be applied)") Integer limit)
    {
        return executeWithLogging(
                "getting workflows for project " + projectId,
                () -> this.workflowApi.getProjectWorkflowAccessContext(projectId).getWorkflows(revisionIds, statuses, limit)
        );
    }
    ...
    public Workflow getWorkflow(@PathParam("projectId") String projectId, @PathParam("workflowId") String workflowId)
    {
        return executeWithLogging(
                "getting workflow " + workflowId + " for project " + projectId,
                () -> this.workflowApi.getProjectWorkflowAccessContext(projectId).getWorkflow(workflowId)
        );
    }
}
