...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/conflictResolution/revisions/{revisionId}/entityPaths")
...
public class ConflictResolutionPatchesWorkspaceRevisionEntityPathsResource extends EntityAccessResource
{
                                       ...
                                       @ApiParam("Only include entities with a matching tagged value. The syntax is PROFILE.NAME/REGEX, where PROFILE is the full path of the Profile that owns the Tag, NAME is the name of the Tag, and REGEX is a regular expression to match against the value.") List<String> taggedValueRegexes)
    {
        ...
        return executeWithLogging(
                "getting entity paths in revision " + revisionId + " of user workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> getEntityPaths(this.entityApi.getWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER, revisionId), classifierPaths, packages, includeSubPackages, nameRegex, stereotypes, taggedValueRegexes)
        );
    }
}
