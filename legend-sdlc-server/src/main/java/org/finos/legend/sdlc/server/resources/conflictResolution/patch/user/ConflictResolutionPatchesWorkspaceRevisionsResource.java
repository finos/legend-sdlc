...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/conflictResolution/revisions")
...
public class ConflictResolutionPatchesWorkspaceRevisionsResource extends BaseResource
{
                                       ...
                                       @QueryParam("limit") Integer limit)
    {
        ...
        return executeWithLogging(
                "getting revision for user workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getWorkspaceWithConflictResolutionRevisionContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).getRevisions(null, ResolvedInstant.getResolvedInstantIfNonNull(since), ResolvedInstant.getResolvedInstantIfNonNull(until), limit)
        );
    }
                                ...
                                @PathParam("revisionId") @ApiParam("Including aliases: head, latest, current, base") String revisionId)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "getting revision " + revisionId + " for user workspace with conflict resolution " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getWorkspaceWithConflictResolutionRevisionContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).getRevision(revisionId)
        );
    }
}
