...
public class GroupWorkspaceRevisionEntitiesResource extends EntityAccessResource
{
    ...
    @ApiOperation("Get invalid entities of the group workspace at the revision")
    ...
    {
        return executeWithLogging(
                "getting invalid entities in revision " + revisionId + " of group workspace " + workspaceId + " for project " + projectId,
                () -> getInvalidEntities(this.entityApi.getGroupWorkspaceRevisionEntityAccessContext(projectId, workspaceId, revisionId))
        );
    }

    @GET
    @Path("invalidEntities/{path}")
    @ApiOperation("Get an invalid entity of the group workspace at the revision by its path")
    ...
    {
        return executeWithLogging(
                "getting invalid entity " + path + " in revision " + revisionId + " of group workspace " + workspaceId + " for project " + projectId,
                () -> this.entityApi.getGroupWorkspaceRevisionEntityAccessContext(projectId, workspaceId, revisionId).getInvalidEntity(path)
        );
    }
}
