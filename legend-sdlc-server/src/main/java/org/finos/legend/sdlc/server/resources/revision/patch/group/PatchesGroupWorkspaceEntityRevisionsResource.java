...
public class PatchesGroupWorkspaceEntityRevisionsResource extends BaseResource
{
                                       ...
                                       @QueryParam("limit") @ApiParam("If not provided or the provided value is non-positive, no filtering will be applied") Integer limit)
    {
        ...
        return executeWithLogging(
                ...
                () -> this.revisionApi.getWorkspaceEntityRevisionContext(projectId, SourceSpecification.newGroupSourceSpecification(workspaceId, versionId), path).getRevisions(null, ResolvedInstant.getResolvedInstantIfNonNull(since), ResolvedInstant.getResolvedInstantIfNonNull(until), limit)
        );
    }
                                ...
                                @PathParam("revisionId") @ApiParam("Including aliases: head, latest, current, base") String revisionId)
    {
        ...
        return executeWithLogging(
                ...
                () -> this.revisionApi.getWorkspaceEntityRevisionContext(projectId, SourceSpecification.newGroupSourceSpecification(workspaceId, versionId), path).getRevision(revisionId)
        );
    }
}
