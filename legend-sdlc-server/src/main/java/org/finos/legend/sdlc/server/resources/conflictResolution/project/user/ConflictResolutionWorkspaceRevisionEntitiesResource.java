...
package org.finos.legend.sdlc.server.resources.conflictResolution.project.user;
...
public class ConflictResolutionWorkspaceRevisionEntitiesResource extends EntityAccessResource
{
                                                  ...
                                                  @ApiParam("Including aliases: head, latest, current, base") String revisionId)
    {
        return executeWithLogging(
                ...
                () -> getInvalidEntities(this.entityApi.getUserWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, workspaceId, revisionId))
        );
    }

    @GET
    @Path("invalidEntities/{path}")
    @ApiOperation("Get an invalid entity of the user workspace with conflict resolution at the revision by its path")
                                                ...
                                                @PathParam("path") String path)
    {
        return executeWithLogging(
                "getting entity " + path + " in revision " + revisionId + " of user workspace with conflict resolution " + workspaceId + " for project " + projectId,
                () -> this.entityApi.getUserWorkspaceWithConflictResolutionRevisionEntityAccessContext(projectId, workspaceId, revisionId).getInvalidEntity(path)
        );
    }
}
