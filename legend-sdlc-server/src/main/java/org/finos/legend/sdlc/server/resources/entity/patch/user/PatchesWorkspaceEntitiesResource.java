...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/entities")
@Api("Entities")
...
public class PatchesWorkspaceEntitiesResource extends EntityAccessResource
{
                                       ...
                                       @ApiParam("If true, exclude invalid entities and return valid entities only. If false, the endpoint will return an error if there are any invalid entities.") boolean excludeInvalid)
    {
        ...
        return execute(
                "getting entities in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                "get entities of the user workspace",
                () -> getEntities(this.entityApi.getWorkspaceEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER), classifierPaths, packages, includeSubPackages, nameRegex, stereotypes, taggedValueRegexes, excludeInvalid)
        );
    }
    ...
    public Revision deleteEntities(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId, DeleteEntitiesCommand command)
    {
        ...
        return (entityPathsToDelete == null) ?
                executeWithLogging(
                        "deleting all entities in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                        () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).deleteAllEntities(command.getMessage())
                ) :
                executeWithLogging(
                        "deleting " + entityPathsToDelete.size() + " entities in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                        () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).deleteEntities(entityPathsToDelete, command.getMessage())
                );
    }
    ...
    public Revision updateEntities(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId, UpdateEntitiesCommand command)
    {
        ...
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return execute(
                "updating entities in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                "update entities",
                () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).updateEntities(command.getEntities(), command.isReplace(), command.getMessage())
        );
    }
    ...
    public Entity getEntityByPath(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId, @PathParam("path") String path)
    {
        ...
        return executeWithLogging(
                "getting entity " + path + " in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.entityApi.getWorkspaceEntityAccessContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).getEntity(path)
        );
    }
    ...
    public Revision createOrUpdateEntity(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId, @PathParam("path") String path, CreateOrUpdateEntityCommand command)
    {
        ...
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "Creating or updating entity " + path + " in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).createOrUpdateEntity(path, command.getClassifierPath(), command.getContent(), command.getMessage())
        );
    }
    ...
    public Revision deleteEntity(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId, @PathParam("path") String path, DeleteEntityCommand command)
    {
        ...
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "deleting entity " + path + " in user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.entityApi.getWorkspaceEntityModificationContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).deleteEntity(path, command.getMessage())
        );
    }
}
