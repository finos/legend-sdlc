...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/groupWorkspaces/{workspaceId}/backup")
@Api("Backup")
...
public class BackupPatchesGroupWorkspaceResource extends BaseResource
{
    ...
    public Workspace getGroupWorkspace(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId)
    {
        ...
        return executeWithLogging(
                "getting backup group workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.workspaceApi.getBackupWorkspace(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP)
        );
    }
    ...
    public boolean isWorkspaceOutdated(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "checking if backup group workspace " + workspaceId + " of project " + projectId + " for patch release version " + patchReleaseVersion + " is outdated",
                () -> this.workspaceApi.isBackupWorkspaceOutdated(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP)
        );
    }
    ...
    public void discardBackup(@PathParam("projectId") String projectId, @PathParam("patchReleaseVersion") String patchReleaseVersion, @PathParam("workspaceId") String workspaceId)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        executeWithLogging(
                "discarding backup group workspace " + workspaceId + " in project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.backupApi.discardBackupWorkspace(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP)
        );
    }
                              ...
                              @QueryParam("forceRecovery") @ApiParam("Whether to override the workspace if it exists with the backup") boolean forceRecovery)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        executeWithLogging(
                forceRecovery ? "force " : "" + "recovering group workspace " + workspaceId + " from backup in project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.backupApi.recoverBackupWorkspace(projectId, patchReleaseVersion, workspaceId, WorkspaceType.GROUP, forceRecovery)
        );
    }
}