...
@Path("/projects/{projectId}/patches/{patchReleaseVersion}/workspaces/{workspaceId}/revisions")
@Api("Revisions")
...
public class PatchesWorkspaceRevisionsResource extends BaseResource
{
                                       ...
                                       @QueryParam("limit") @ApiParam("If not provided or the provided value is non-positive, no filtering will be applied") Integer limit)
    {
        ...
        return executeWithLogging(
                "getting revision for user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getWorkspaceRevisionContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).getRevisions(null, ResolvedInstant.getResolvedInstantIfNonNull(since), ResolvedInstant.getResolvedInstantIfNonNull(until), limit)
        );
    }
                                ...
                                @PathParam("revisionId") @ApiParam("Including aliases: head, latest, current, base") String revisionId)
    {
        LegendSDLCServerException.validateNonNull(patchReleaseVersion, "patchReleaseVersion may not be null");
        return executeWithLogging(
                "getting revision " + revisionId + " for user workspace " + workspaceId + " for project " + projectId + " for patch release version " + patchReleaseVersion,
                () -> this.revisionApi.getWorkspaceRevisionContext(projectId, patchReleaseVersion, workspaceId, WorkspaceType.USER).getRevision(revisionId)
        );
    }
}
