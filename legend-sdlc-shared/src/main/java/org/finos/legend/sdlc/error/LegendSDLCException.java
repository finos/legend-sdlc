...
/**
 * Base exception for Legend SDLC. It carries an HTTP-style status code as a plain int, so that it can be thrown by
 * code with no dependency on any web framework. Server code that needs a framework status type (e.g. the JAX-RS
 * {@code Response.Status} enum) can convert from {@link #getStatusCode()}.
 */
public class LegendSDLCException extends RuntimeException
{
    private static final long serialVersionUID = 6730387395611537718L;

    private static final int DEFAULT_STATUS_CODE = 500;
    private static final int DEFAULT_VALIDATION_STATUS_CODE = 400;
