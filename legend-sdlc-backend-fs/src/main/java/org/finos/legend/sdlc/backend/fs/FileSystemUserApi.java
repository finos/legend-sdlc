...
/**
 * The file-system backend has no user directory: the only known user is the session's. {@code getUserById}
 * echoes any requested id back as a user (ids are not validated against a directory that does not exist).
 */
public class FileSystemUserApi implements UserApi
{
    private final String userId;

    FileSystemUserApi(String userId)
    {
        this.userId = userId;
    }
